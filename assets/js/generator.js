...
const Generator = {
  ...
  async build(config) {
    ...
    zip.file('academic-records.html', Generator.pageAcademicRecords(cfg)); // record card + broadsheets
...
const App = {
  ...
  applyRoleVisibility() {
    if (!sb) { App.applyRoleDashboard('demo', { full_name:'Demo User', role:'admin' }); return; }
  ...
  applyRoleDashboard(role, profile) {
    ...
    if(q){
      const links = role==='parent' ? [['My Child','student-profile.html'],['Fees','fees.html'],['Messages','inbox.html']] :
        (['staff','teacher'].includes(role) ? [['Attendance','attendance.html'],['CBT','cbt.html'],['Report Cards','report-cards.html']] :
        [['Students','students.html'],['Approvals','approvals.html'],['Analytics','analytics.html'],['Storage','storage.html']]);
  ...
  pageCBT(cfg) {
    const body = `
      <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px" data-staff-only>
        ...
        <button class="btn btn-outline" onclick="CBTUI.promptGuide()">🧩 Question Prompt Guide</button>
...
const CBTUI = {
  ...
  promptGuide(){
    const txt = 'Use this structured teacher prompt to manually generate a CSV question bank. No runtime AI API is used by School Connect.\n\nROLE: Act as an expert examiner and curriculum-aligned question setter.\nTASK: Generate a CSV question bank for School Connect CBT.\nOUTPUT RULES:\n1. Return CSV only, no explanations outside the CSV.\n2. Use headers exactly: Question,A,B,C,D,CorrectAnswer,Explanation,Type,Tolerance,Unit,Accept,MRQ_AON,Pairs,Items,Difficulty,Tags,Section\n3. Support these types: mcq,truefalse,numeric,short,mrq,essay,matching,ordering,cloze,categorization,multinumeric,assertion_reason,case_study,image_mcq,matrix,hot_text,code.\n4. For MCQ use CorrectAnswer A/B/C/D. For MRQ use A|C format. For matching use Pairs as left:right|left:right. For advanced structured types use Items JSON.\n5. Include clear explanations for teacher review.\n6. Balance difficulty: 30% Easy, 50% Medium, 20% Hard.\n\nEXAM DETAILS TO FILL:\nSubject: [SUBJECT]\nClass/Level: [CLASS]\nTerm/Session: [TERM, SESSION]\nTopic(s): [TOPICS]\nNumber of questions: [COUNT]\nAssessment standard: Nigerian + international best practice.\n';
...
document.addEventListener('DOMContentLoaded',()=>FLUI.render()); FLUI.render();
  ...
  fullPreviewHtml(config) {
    ...
    const dedicated = {
      ...
      'student-profile': () => Generator.pageStudentProfile(cfg), academic_records: () => Generator.pageAcademicRecords(cfg),
